module.exports = function(mongo) {
  ...
  function authCallback(avatarCallback){
    return function(token, tokenSecret, profile, done) {
      mongo.collection('users').findAndModify({ provider: profile.provider, providerId: profile.id }, [], { $setOnInsert: { name: profile.displayName, permissionsMask: 1 } }, { new: true, upsert: true },
        function(err, item){
